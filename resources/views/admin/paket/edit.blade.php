    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('admin.paket.update', $paket) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="harga" id="harga" value="{{ old('harga', $paket->harga) }}"
                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    required>
                @error('harga')
                    <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="tanggal_keberangkatan" id="tanggal_keberangkatan"
                    value="{{ old('tanggal_keberangkatan', \Carbon\Carbon::parse($paket->tanggal_keberangkatan)->format('Y-m-d')) }}"
                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    required>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="kuota" id="kuota" value="{{ old('kuota', $paket->kuota) }}"
                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    required>
                @error('kuota')
                    <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
            ...
            <div class="flex items-center justify-end gap-2">
                <a href="{{ route('admin.paket.index') }}" class="px-4 py-2 bg-red-600 rounded-md text-white font-medium tracking-wide hover:bg-red-500 ml-3">Batal</a>
