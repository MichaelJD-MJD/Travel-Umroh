        ...
        <div class="block space-y-4">
            <div class="mt-6">
                <div class="flex justify-between">
                    <h2 class="text-3xl font-bold text-gray-700 leading-tight">Daftar Galeri</h2>
                    ...
                    <div class="inline-block min-w-full shadow rounded-lg overflow-hidden">
                                ...
                                @forelse ($galeris as $galeri)
                                    <tr class="border-b border-gray-200 hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-sm">
                                            <a href="{{ route('admin.galeri.edit', $galeri->galeri_id) }}"
                                              ...
                                              <div x-data="{ showConfirm: false }" class="inline-block ml-3">
                                                ...
                                                <div x-show="showConfirm" x-transition
                                                    ...
                                                    <div @click.away="showConfirm = false" class="bg-white rounded-lg shadow-lg p-6 ">
                                                        ...
                                                        <p class="text-gray-600 mb-6">Apakah Anda yakin ingin menghapus Galeri <strong>{{ $galeri->judul }}</strong>?</p>
                                                        <div class="flex justify-end space-x-3 gap-2">
                                                            ...
                                                            <form action="{{ route('admin.galeri.destroy', $galeri->galeri_id) }}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit"
                        ...
                        <div
                            ...
                            <span class="text-xs text-gray-900">
                                Showing {{ $galeris->firstItem() }} to {{ $galeris->lastItem() }} of
                                {{ $galeris->total() }} Entries
                            </span>
                            <div class="inline-flex mt-2 xs:mt-0">
                                <a href="{{ $galeris->previousPageUrl() ?? '#' }}"
                                    class="text-sm bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-2 px-4 rounded-l 
                                   {{ $galeris->previousPageUrl() ? 'focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50' : 'opacity-50 cursor-not-allowed' }}">
                                    Prev
                                </a>
                                <a href="{{ $galeris->nextPageUrl() ?? '#' }}"
                                    class="text-sm bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-2 px-4 rounded-r
                                   {{ $galeris->nextPageUrl() ? 'focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50' : 'opacity-50 cursor-not-allowed' }}">
                                    Next
                                </a>
                            </div>
                        </div>
